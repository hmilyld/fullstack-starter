...
public class AuthService {
    ...
    public ApiResponse<?> login(String account, String password) {
        ...
        if (Boolean.TRUE.equals(config.getMaintenanceEnabled()) && !"admin".equals(user.getRoleId())) {
            return ApiResponse.error(config.getMaintenanceMessage() != null && !config.getMaintenanceMessage().isEmpty()
                    ? config.getMaintenanceMessage() : "系统维护中");
        }
    ...
    public ApiResponse<?> register(String username, String email, String password) {
        ...
        List<String> permissions = rolePermissionRepository.findPermissionCodesByRoleId(user.getRoleId());
